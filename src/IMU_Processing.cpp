    ...
    : b_first_frame_(true), imu_need_init_(true), start_timestamp_(-1)
{
    ...
    cov_acc_scale = Eigen::Vector3d(1, 1, 1);
    cov_gyr_scale = Eigen::Vector3d(1, 1, 1);
    cov_bias_gyr = Eigen::Vector3d(0.1, 0.1, 0.1);
    cov_bias_acc = Eigen::Vector3d(0.1, 0.1, 0.1);
...
{
    // Eigen::Vector3d acc_tmp(last_acc), angvel_tmp(last_ang), vel_imu(state.vel_end), pos_imu(state.pos_end);
...
void ImuProcess::set_gyr_cov_scale(const Eigen::Vector3d &scaler)
{
    cov_gyr_scale = scaler;
}

void ImuProcess::set_acc_cov_scale(const Eigen::Vector3d &scaler)
{
...
void ImuProcess::Forward(const MeasureGroup &meas, StatesGroup &state_inout, double pcl_beg_time, double end_time)
{
  ...
  //          <<meas.imu.size()<<" imu msgs from "<<imu_beg_time<<" to "<<imu_end_time<<endl;
    ...
    state_inout.rot_end = R_imu * Exp(Eigen::Vector3d(note * angvel_avr), dt);
...
void ImuProcess::Backward(const LidarMeasureGroup &lidar_meas, StatesGroup &state_inout, PointCloudXYZI &pcl_out)
{
    ...
    for (auto it_kp = IMUpose.end() - 1; it_kp != IMUpose.begin(); it_kp--)
    {
        ...
        for (; it_pcl->curvature / double(1000) > head->offset_time; it_pcl--)
        {
            ...
            Eigen::Vector3d T_ei(pos_imu + vel_imu * dt + 0.5 * acc_imu * dt * dt + R_i * Lid_offset_to_IMU - pos_liD_e);
